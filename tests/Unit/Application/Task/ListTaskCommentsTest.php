...
function makeListTaskCommentsFixture(): array
{
    ...
    $useCase = new ListTaskComments($comments, $tasks, $members);

    return ['useCase' => $useCase, 'alice' => $alice, 'eve' => $eve, 't1' => $t1, 't2' => $t2];
}

it('returns comments for a task in id order', function (): void {
    $ctx = makeListTaskCommentsFixture();

    $result = $ctx['useCase']->execute($ctx['alice']->id, $ctx['t1']->id);
    $bodies = array_map(fn ($c): string => $c->body, $result);

    expect($bodies)->toBe(['first', 'second']);
});

it('rejects listing comments for unknown task', function (): void {
    $ctx = makeListTaskCommentsFixture();

    $ctx['useCase']->execute($ctx['alice']->id, 999);
})->throws(TaskNotFoundException::class);

it('does not return comments from other tasks', function (): void {
    $ctx = makeListTaskCommentsFixture();

    $t2Comments = $ctx['useCase']->execute($ctx['alice']->id, $ctx['t2']->id);
    expect($t2Comments)->toHaveCount(1)
        ->and($t2Comments[0]->body)->toBe('on t2');
});

it('rejects listing comments when actor is not a project member', function (): void {
    $ctx = makeListTaskCommentsFixture();

    $ctx['useCase']->execute($ctx['eve']->id, $ctx['t1']->id);
